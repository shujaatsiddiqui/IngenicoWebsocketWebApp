...
HANDSHAKE_401_STR = (
    ...
    "Server: nginx\r\n"
    "Date: Mon, 01 Mar 2021 17:50:34 GMT\r\n"
    "Content-Type: text/html\r\n"
    "Content-Length: 0\r\n"
    "Connection: keep-alive\r\n"
    "WWW-Authenticate: Bearer realm=\"Paymentus Zone\", error=\"invalid_token\"\r\n"
    "X-debug-message: Matched IGO connect eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiIsImtpZCI6IjAwMSJ9.eyJjbGllbnQiOiI4MDY1NjY2NzAwIiwiaHNuIjoiMjAyNDI3MzEzMDkxMDQxMTE2NjE4NjA1IiwiaXNzIjoiaWdvLXRlcm1pbmFsLXNlcnZpY2UiLCJpYXQiOjE2MTQ2MjEwMzQsImV4cCI6MTYxNDYyMTYzNH0.xV1fRLIvHIxbfZHvuWfgJD4kmPS6lHHj5Q_fm55dfbI_\r\n\r\n"
)
...
MAXHEADER = 65536
#MAXPAYLOAD = 33554432 original 32 MB
MAXPAYLOAD = 52428800 # 50 MB


class WebSocket(object):

    http_401_probability = 0

    def __init__(self, server, sock, address, pongdisabled):
        self.server = server
        self.client = sock
        ...
        self.pongdisabled = pongdisabled
    ...
    def _handlePacket(self):
        ...
        if self.opcode == CLOSE:
            ...
            if self.opcode == STREAM:
                ...
                if self.pongdisabled == 0:
                    print("PING RECEIVED - SENDING PONG to: ", self.request.path.lower())
                    self._sendMessage(False, PONG, self.data)
                else:
                    print("PING RECEIVED - DO NOT SENDING PONG to: ", self.request.path.lower())
            ...
            if not data:
                ...
                if b'\r\n\r\n' in self.headerbuffer:
                    ...
                    try:
                        ...
                        portNumber = self.client.getsockname()[1]
                        isRegistration = portNumber == 50002                        
                        if 0 < WebSocket.http_401_probability > rand_choice and not (isRegistration) :
                            print("=== Requested %s responding with HTTP/1.1 401 Unauthorized" % self.request.path)
                            hStr = HANDSHAKE_401_STR
                            self.sendq.append((BINARY, hStr.encode('ascii')))
                            self.handshaked = False
                            self.close()
                        else:
                            #print("=== Requested %s responding with HTTP/1.1 101 Switching Protocols" % self.request.path)
    ...
    def _parseMessage(self, byte):
        ...
        # PAYLOAD STATE
            ...
            if len(self.data) >= self.maxpayload:
                print("PAYLOAD payload exceeded allowable size")
...
class SimpleWebSocketServer(object):
    ...
    def __init__(self, host, port, websocketclass, pongdisabled, selectInterval=0.1):
    ...
    def _constructWebSocket(self, sock, address, pongdisabled):
        return self.websocketclass(self, sock, address, pongdisabled)
        ...
        for ready in rList:
            if ready == self.serversocket:
                try:
                    ...
                    self.connections[fileno] = self._constructWebSocket(newsock, address, self.pongdisabled)
...
class SimpleSSLWebSocketServer(SimpleWebSocketServer):
    ...
    def __init__(self, host, port, websocketclass, certfile,
                 keyfile, version=ssl.PROTOCOL_TLSv1, selectInterval=0.1, ciphers="ALL"):
    ...
    def _constructWebSocket(self, sock, address, pongdisabled):
        ws = self.websocketclass(self, sock, address, pongdisabled)
        ws.usingssl = True
        return ws
