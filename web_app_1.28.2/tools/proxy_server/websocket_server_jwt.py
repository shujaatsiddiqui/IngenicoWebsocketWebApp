...
class API():
	...
	def set_fn_close_client_connection(self, fn):
		self.close_client_connection = fn



# ------------------------- Implementation -----------------------------

class WebsocketServerJwt(ThreadingMixIn, TCPServer, API):
	...
	def _new_client_(self, handler, path):
		self.id_counter += 1
		client={
			'id'      : self.id_counter,
			'handler' : handler,
	...
	def handler_to_client(self, handler):
		for client in self.clients:
			if client['handler'] == handler:
				return client
	def _close_client_connection_(self):
...
class WebSocketHandler(StreamRequestHandler):
	...
	def read_next_message(self):
		...
		if opcode == CLOSE_CONN:
			...
			self.keep_alive = 0
			self.server._close_client_connection_()
			return
		if not masked:
	...
	def handshake(self):
		...
		if key:
			...
			self.keep_alive = False
			return
		path = re.search('^GET (.+) HTTP/1.1', message)
		response = self.make_handshake_response(key)
		self.handshake_done = self.request.send(response.encode())
		self.valid_client = True
		self.server._new_client_(self, path.group(1))
